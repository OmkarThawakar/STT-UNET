...
class Trainer(object):
            ...
            for _, (pos, volume) in enumerate(self.dataloader):
                ...
                if not "super" in self.cfg.MODEL.ARCHITECTURE:
                    ...
                    for idx in range(output.shape[0]):
                        ...
                        weight[st[0]][st[1]:st[1]+sz[1], st[2]:st[2]+sz[2], \
                        st[3]:st[3]+sz[3]] += ww
                # print("breaking dataloader loop ::::")    
                # break
    ...
    def save_checkpoint(self, iteration, cfg):
        ...
        del self
        gc.collect()
        cal_infer(root_dir=cfg.DATASET.INFERENCE_PATH, model_id=(iteration + 1), output_path=cfg.DATASET.OUTPUT_PATH)
    ...
    def update_checkpoint(self, checkpoint):
        ...
        if not self.cfg.SOLVER.ITERATION_RESTART:
            ...
            if 'optimizer' in checkpoint.keys():
                # print("checkpoint['optimizer'] :::: ", checkpoint['optimizer'].keys())
                # print('='*50)
                # print('self.optimizer :::: ', self.optimizer.keys())
                # print('='*50)
